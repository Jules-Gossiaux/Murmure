...
- **Sélecteurs** : la molette de la souris est désactivée sur tous les sélecteurs afin d’éviter une modification accidentelle pendant le défilement. Utilisez la liste déroulante ou les touches fléchées après l’avoir ouverte.
...
Dans les paramètres, **Corriger automatiquement le texte** active un second modèle local compact. Murmure utilise **Qwen2.5-0.5B-Instruct Q4_K_M** (environ 491 Mo, CPU, licence Apache-2.0) pour corriger l’orthographe, la grammaire et les mots manifestement mal transcrits. Le bouton **Télécharger le correcteur maintenant** permet de le préparer avant la première dictée ; il est ensuite disponible hors ligne dans le dossier des modèles. La correction est désactivée par défaut et n’ajoute aucun délai lorsqu’elle est désactivée.
